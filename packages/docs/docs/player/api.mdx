...
This option controls how many audio tags are being rendered, the default is `5` in Remotion 4.0 and `0` in Remotion 5.0. If you mount more audio tags than shared audio tags are available, then an error will be thrown.

If you want to mount native audio tags directly without using the shared tag pool, you can pass `0` to the `<Player>`.
