...
On `G`, these controls set inherited paint defaults for descendants.
The root `Svg` element exposes a `color` control instead of `fill`, because `fill` has no effect on the root; `color` is the inherited presentation property that drives `currentColor` in descendants.
