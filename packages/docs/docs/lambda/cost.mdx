...
Using [`<Video />`](/docs/media/video) and [`<Audio />`](/docs/media/audio) from [`@remotion/media`](/docs/media) can reduce the cost of renders on Lambda.

The `@remotion/media` tags render faster than [`<OffthreadVideo />`](/docs/offthreadvideo) and use partial downloads, so the Lambda function only fetches the parts of a media file that are needed instead of downloading the whole file. See the [data transfer cost](/docs/lambda/data-transfer-cost) page and the [comparison of video tags](/docs/video-tags) for more details.
