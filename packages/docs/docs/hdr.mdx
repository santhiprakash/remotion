...
In Remotion 5, `angle` is already the default renderer during rendering, so you usually do not need to pass `--gl=angle` just for HDR playback.

If you are on Remotion 4 or want to override the renderer explicitly, `--gl=angle` can still improve color handling when embedding an HDR video (tested on macOS).  
We have not yet experimented with this enough to give a recommendation on which `--gl` flag to use for all setups.  
