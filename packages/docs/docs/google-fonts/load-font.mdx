...
:::warning
Calling `loadFont()` without `weights` and `subsets` loads all styles, weights and character subsets of a font. This can trigger a lot of network requests and may lead to warnings or timeouts. In Remotion 5.0, `weights` and `subsets` are required — see the [v5 migration guide](/docs/5-0-migration).
:::
...
In Remotion 4.0, all arguments and options are optional. Use TypeScript autocompletion or [`getInfo`](/docs/google-fonts/get-info) to discover available options. If no arguments are passed, all styles, weights and subsets are loaded.

In Remotion 5.0, you must specify `weights` and `subsets` in the options object. Loading all weights and subsets by default is no longer supported. See the [v5 migration guide](/docs/5-0-migration).
...
Array of weights that should be loaded. Required in Remotion 5.0. By default, all in Remotion 4.0.

#### subsets?

Array of font subsets that should be loaded. Required in Remotion 5.0. By default, all in Remotion 4.0.
