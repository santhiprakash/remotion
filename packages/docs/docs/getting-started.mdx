...
New Remotion projects come with ESLint configured automatically using [`@remotion/eslint-config`](/docs/eslint-config). If you are adding Remotion to an existing project or want to customize the rules, see the [`@remotion/eslint-plugin`](/docs/eslint-plugin) documentation.
